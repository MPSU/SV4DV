# Лабораторная работа 1 "SystemVerilog: Задачи, функции, программы. Параллельные процессы, примитивы синхронизации, автоматические переменные и таски. "
...
- [Лабораторная работа 1 "SystemVerilog: Задачи, функции, программы. Параллельные процессы, примитивы синхронизации, автоматические переменные и таски. "](#лабораторная-работа-1-systemverilog-задачи-функции-программы-параллельные-процессы-примитивы-синхронизации-автоматические-переменные-и-таски-)
  - [Цель](#цель)
  ...
  - [Теория](#теория)
    ...
    - [2. SystemVerilog и параллельные процессы](#2-systemverilog-и-параллельные-процессы)
      ...
      - [2.3. Параллельное, но не совсем](#23-параллельное-но-не-совсем)
    ...
    - [6. SystemVerilog задачи (tasks) и функции](#6-systemverilog-задачи-tasks-и-функции)
...
В процессе дальнейшего изучения данной лабораторной работы Вам понадобятся знания о нескольких новых конструкциях, введенных в SystemVerilog.

Более подробно вы можете ознакомиться с ними в [SystemVerilog LRM](../.docs/lab_02/IEEE_Std1800-2017_8299595.pdf), в этом же разделе будут просто даны поясняющие комментарии.
...
Служит для реализации последовательной логики, указывает на то, что реализуется триггер. `always_ff @(posedge clk)` является аналогом `always @(posedge clk)`.
...
Несмотря на то, что в настоящее время SystemVerilog применяется не только для описания поведения цифровых устройств, все базовые конструкции были созданы “с оглядкой” именно на это, а остальные являются наследием базовых.

|Именно поэтому все процессы, выполняющиеся в конкретный момент времени симуляции, выполняются параллельно, имитируя работу реальных цифровых устройств.|
|---|
...
#### 2.3. Параллельное, но не совсем

Ранее было сказано, что, для конкретного момента времени, все процессы языка выполняются параллельно, причем порядок их выполнения не определен. 

Кажется логичным, что порядок выполнения событий в таких процессах тоже не определен. Однако, это далеко не так.
...
Ключевые слова `timeunit` и `timeprecision` должны предшествовать любым другим элементам модуля, в котором они указаны. Также, объявление слов может повторяться в коде модуля позже, но они значение временных единиц должны повторять значения из предыдущего объявления.
...
|Переменная типа `event` называется именованным событием. Именованное событие представляет собой контейнер для объекта синхронизации.|
|---|
...
|`Блочные выражения` или `блоки` – средство группировки конструкций вместе таким образом, чтобы они синтаксически они выполняли единую функцию.|
|---|
...
### 6. SystemVerilog задачи (tasks) и функции

|Задачи (`tasks`) и функции (`functions`) являются конструкциями языка SystemVerilog, которые позволяют объединять и использовать общие процедуры из различных участков кода. Они также позволяют разбивать большие процедуры на более мелкие, чтобы облегчить чтение и отладку.|
|---|
...
`Значения аргументов` между вызовами также не сохраняются, а принимают базовые значения своих типов.

`Локальные переменные` могут быть объявлены как автоматические в статической задаче или как статические в автоматической задаче.
...
|В теле функции может быть написано несколько выражений. `Выражения выполняются последовательно`, так же, как если бы они были заключены в последовательный блок.|
|---|

`Тело функции может не иметь выражений вовсе`, тогда функцией будет возвращено базовое значение переменной типа, который возвращает функция.
...
Возвращаемое функцией значение можно определить двумя способами:
  - либо с помощью оператора `return`;
...
`Значения аргументов` также не сохраняются, а принимают базовые значения своих типов.

`Конкретные локальные` переменные могут быть объявлены как автоматические в статической функции или как статические в автоматической функции.
